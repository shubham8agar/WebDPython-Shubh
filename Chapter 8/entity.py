...
# TODO: edit processing should process related stuff as well
...
class AbstractEntity(metaclass=MetaMetaEntity):
	...
	def listids(cls,pattern=None,sortorder=None):
		...
		if not pattern is None and len(pattern)>0:
			...
			if len(filterattrs) or len(joins):
				sql += " where "
			if len(filterattrs):
				sql += " and ".join("%s like ?"%s[0] for s in filterattrs)
				args += [s[1] for s in filterattrs]
			if len(joins):
				sql += " "
				if len(filterattrs): sql += "and "
	...
	def __new__(metaclass,classname,baseclasses,classdict):
		...
		if len(baseclasses):
			...
			if relationdefinition or '_meta' in classdict:
				...
				if not r in ('N:1','1:N'): raise KeyError("unknown relation_type %s"%r)
