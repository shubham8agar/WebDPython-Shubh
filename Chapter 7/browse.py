...
class Browse:
	...
	@cherrypy.expose
	def index(self, _=None, start=0, pattern=None, sortorder=None, cacheid=None, next=None,previous=None, first=None, last=None, clear=None):
		#print(cherrypy.serving.request.headers)
		
		if not clear is None: pattern=None
			...
			if type(col) == str :
				...
				for s in sortorder:
					if s[0]==col :
						...
						break
				yield '<th class="%s">'%sortclass+self.entity.displaynames[col]+'</th>'
			else :
		...
		for e in entities:
			...
			print(vals)
			yield ('<tr id="%d"><td>'+'</td><td>'.join(vals)+'</td></tr>\n')%(e.id,)
		yield '</tbody>\n</table>\n'
		# yield nav buttons
		...
		yield '<form method="GET" action="add"><button type="submit">Add new</button></form>'
